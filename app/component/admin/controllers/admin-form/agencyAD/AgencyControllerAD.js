travel_app.controller('AgencyControllerAD',
    function ($scope, $http, $location, $sce, $rootScope, $routeParams, $timeout, AgencyServiceAD) {
        ...
        /**
         * Find all tất cả dữ liệu ra ngoài
         */
        $scope.getTypeList = function () {
            $scope.isLoading = true;

            AgencyServiceAD.findAllType($scope.currentPage, $scope.pageSize, $scope.sortBy, $scope.sortDir).then(function (response) {
            ...
            if (typeId !== undefined && typeId !== null && typeId !== "") {
                AgencyServiceAD.findAgencieById(typeId).then(function successCallback(response) {
        ...
        $scope.searchTypes = function () {
            ...
            searchTimeout = $timeout(function () {
                AgencyServiceAD.findAllType($scope.currentPage, $scope.pageSize, $scope.sortBy, $scope.sortDir, $scope.searchTerm).then(function (response) {
                    if (response.status === 200) {
                        $scope.typeList = response.data.data.content;
                        $scope.totalPages = Math.ceil(response.data.data.totalElements / $scope.pageSize);
                        $scope.totalElements = response.data.data.totalElements;
