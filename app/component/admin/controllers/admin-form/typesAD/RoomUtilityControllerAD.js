travel_app.controller('RoomUtilityControllerAD',
    function ($scope, $location, $sce, $rootScope, $routeParams, $timeout, RoomUtilityServiceAD, Base64ObjectService) {
        ...
        $scope.getTypeList = function () {
            ...
            RoomUtilityServiceAD.findAllType($scope.currentPage, $scope.pageSize, $scope.sortBy, $scope.sortDir)
            ...
            if (typeId !== undefined && typeId !== null && typeId !== "") {
                RoomUtilityServiceAD.findById(typeId)
                    .then(function successCallback(response) {
                        if (response.status === 200) {
                            $scope.formType = response.data.data;
                            $rootScope.namenow = $scope.formType.roomUtilitiesName;
                        }
                    }, errorCallback).finally(function () {
                    $scope.isLoading = false;
                });
        ...
        $scope.searchTypes = function () {
            ...
            searchTimeout = $timeout(function () {
                RoomUtilityServiceAD.findAllType($scope.currentPage, $scope.pageSize, $scope.sortBy, $scope.sortDir, $scope.searchTerm)
                    .then(function (response) {
                        if (response.data.status === "404") {
                            $scope.typeList.length = 0;
                        } else {
        ...
        $scope.checkDuplicateTypeName = function () {
            RoomUtilityServiceAD.checkExistTypeName($scope.formType.roomUtilitiesName)
                .then(function successCallback(response) {
                    if (response.status === 200) {
        ...
        $scope.checkDuplicateTypeNameUpdate = function () {
            if ($scope.formType.roomUtilitiesName === $rootScope.namenow) {
        ...
        $scope.createType = function () {
            ...
            RoomUtilityServiceAD.createThisType(dataType).then(function successCallback() {
                toastAlert('success', 'Thêm mới thành công !');
                $location.path('/admin/type/room-utility-list');
            }, errorCallback).finally(function () {
                $scope.isLoading = false;
            });
        }
        ...
        function confirmUpdateType() {
            $scope.isLoading = true;
            RoomUtilityServiceAD.updateThisType($scope.formType).then(function successCallback() {
        ...
        $scope.deleteType = function (typeId) {
            function confirmDeleteType() {
                ...
                RoomUtilityServiceAD.checkTypeIsWorking(typeId).then(
                    function successCallback(response) {
                        if (response.data.status.toString() === "200") {
                            toastAlert('error', "Thể loại đang được sử dụng !");
                        } else {
                            RoomUtilityServiceAD.deleteThisType(typeId)
                                .then(function () {
