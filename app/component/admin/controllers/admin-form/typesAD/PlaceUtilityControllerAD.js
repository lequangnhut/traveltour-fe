travel_app.controller('PlaceUtilityControllerAD',
    function ($scope, $location, $sce, $rootScope, $routeParams, $timeout, PlaceUtilityServiceAD, Base64ObjectService) {
        ...
        $scope.getTypeList = function () {
            ...
            PlaceUtilityServiceAD.findAllType($scope.currentPage, $scope.pageSize, $scope.sortBy, $scope.sortDir)
            ...
            if (typeId !== undefined && typeId !== null && typeId !== "") {
                PlaceUtilityServiceAD.findById(typeId)
                    .then(function successCallback(response) {
                        if (response.status === 200) {
                            $scope.formType = response.data.data;
                            $rootScope.namenow = $scope.formType.placeUtilitiesName;
                        }
                    }, errorCallback).finally(function () {
                    $scope.isLoading = false;
                });
        ...
        $scope.searchTypes = function () {
            ...
            searchTimeout = $timeout(function () {
                PlaceUtilityServiceAD.findAllType($scope.currentPage, $scope.pageSize, $scope.sortBy, $scope.sortDir, $scope.searchTerm)
                    .then(function (response) {
                        if (response.data.status === "404") {
                            $scope.typeList.length = 0;
                        } else {
        ...
        $scope.checkDuplicateTypeName = function () {
            PlaceUtilityServiceAD.checkExistTypeName($scope.formType.placeUtilitiesName)
                .then(function successCallback(response) {
                    if (response.status === 200) {
        ...
        $scope.checkDuplicateTypeNameUpdate = function () {
            if ($scope.formType.placeUtilitiesName === $rootScope.namenow) {
        ...
        $scope.createType = function () {
            ...
            PlaceUtilityServiceAD.createThisType(dataType).then(function successCallback() {
                toastAlert('success', 'Thêm mới thành công !');
                $location.path('/admin/type/hotel-utility-list');
            }, errorCallback).finally(function () {
                $scope.isLoading = false;
            });
        }
        ...
        function confirmUpdateType() {
            $scope.isLoading = true;
            PlaceUtilityServiceAD.updateThisType($scope.formType).then(function successCallback() {
        ...
        $scope.deleteType = function (typeId) {
            function confirmDeleteType() {
                ...
                PlaceUtilityServiceAD.checkTypeIsWorking(typeId).then(
                    function successCallback(response) {
                        if (response.data.status.toString() === "200") {
                            toastAlert('error', "Thể loại đang được sử dụng !");
                        } else {
                            PlaceUtilityServiceAD.deleteThisType(typeId)
                                .then(function () {
